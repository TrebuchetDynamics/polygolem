...
```bash
polygolem analytics positions --user 0xDEPOSIT_WALLET --limit 50 --json
```
...
polygolem wallet approve-adapters --submit --confirm APPROVE_ADAPTERS --json
...
polygolem wallet redeem --submit --confirm REDEEM_WINNERS --json
```
...
- [Data API](/docs/reference/data-api)
