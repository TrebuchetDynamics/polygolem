...
> skipped. `polygolem wallet status --json` exposes which path
...
The operator surface is `polygolem wallet approve-adapters`,
...
First-time setup can start directly with `polygolem wallet onboard`.
...
SIGNER_PRIVATE_KEY="0x..." \
  polygolem wallet onboard --fund-amount 50 --confirm ONBOARD_WALLET --json
```
...
to the Polygolem env file, and continue. `polygolem credentials login` is still
...
For pre-provisioned CLOB credentials, run `polygolem builder-keys auto`. Otherwise
...
- `relayer_and_polygon_code` — both reported not deployed. Real first-deploy
  state; `polygolem wallet deploy --wait` will submit `WALLET-CREATE`
