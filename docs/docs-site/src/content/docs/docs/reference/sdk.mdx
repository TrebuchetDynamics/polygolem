...
[Internal Packages](/docs/reference/internal-packages), but they do not carry the
...
func (c *UserClient) Stats() StreamStatsSnapshot
...
`pkg/stream` covers public market data and authenticated user order/trade
streams. Set `Config.CustomFeatureEnabled = true` to request top-of-book and
market lifecycle events from the V2 market channel. `MarketClient.OnRawMessage`
exposes each deduplicated market payload for append-only JSONB recorders, and
`Stats()` includes `last_message_at` plus per-event `event_counts` for
coverage/freshness receipts.

## `pkg/rtds` — Real-Time Data Service Chainlink Prices

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/rtds"
```
...
`pkg/rtds` connects to Polymarket's Real-Time Data Service and exposes the
`crypto_prices_chainlink` feed used by crypto Up/Down markets.
...
The CLI surface is `polygolem prices live --asset-ids ...`, which wires
...
`CategoryEvents` uses the curated polymarket.com category mapping in `pkg/gamma`
and Gamma `GET /events/keyset` feeds. It does not assume a single website
category-list API exists (`pkg/gamma/categories.go:17` to
`pkg/gamma/categories.go:74`, `internal/gamma/categories.go:36` to
`internal/gamma/categories.go:68`).

## `pkg/geoblock` — Geoblock Verdict

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/geoblock"
```

Read-only client for Polymarket's public geoblock endpoint. It reports whether
the caller's IP is blocked from trading and where it appears to be
(`pkg/geoblock/geoblock.go:1`).
...
## `pkg/marketresults` — Authoritative Market Results

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/marketresults"
```

`NewResolver(clobBaseURL, gammaBaseURL)` combines public CLOB winner data with
Gamma's closed timestamp. `Resolve(ctx, conditionID)` returns `nil` until both
sources prove a complete result; transport errors fail closed.
...
## `pkg/wallet` — Deposit-Wallet Identity

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/wallet"
```

Small identity/readiness module for deterministic deposit-wallet derivation.
Deploy, approvals, funding, and settlement stay in their own packages
(`pkg/wallet/doc.go:1`, `pkg/wallet/derive.go:24`).

```go
func DeriveDepositWallet(eoa string) (string, error)
func DeriveProxyWallet(eoa string) string // deprecated
func DeriveSafeWallet(eoa string) string  // deprecated
...
## `pkg/funding` — Explicit pUSD Funding Helper

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/funding"
```

This package submits Polygon mainnet transactions and must only be called after
caller-owned live gates pass (`pkg/funding/funding.go:1`, `pkg/funding/funding.go:17`).
...
## `pkg/cryptoprice` — Crypto Reference Prices

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/cryptoprice"
```

Read-only client for Polymarket web crypto reference prices. It fetches the
open/close reference price backing crypto Up/Down market resolution and does no
signing (`pkg/cryptoprice/client.go:1`, `pkg/cryptoprice/client.go:69`).
...
## `pkg/intel` — Wallet Intelligence DTOs and Scoring

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/intel"
```

Pure read-only DTOs and scoring helpers for wallet dossiers. `ScoreWallet`
returns deterministic, explainable wallet scores from public metrics
(`pkg/intel/types.go:1`, `pkg/intel/score.go:50`).
...
## `pkg/orderfills` — On-Chain Fill Truth Models

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/orderfills"
```

Typed `OrderFilled` reader contract and validators for on-chain fill evidence
(`pkg/orderfills/orderfills.go:1`, `pkg/orderfills/orderfills.go:50`).
...
## `pkg/orderresults` — Joined Order Result Reports

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/orderresults"
```

Builds one operator report from Data API positions/results plus optional CLOB
account history (`pkg/orderresults/orderresults.go:1`, `pkg/orderresults/orderresults.go:30`).
...
## `pkg/rfq` — RFQ DTOs and Guarded Submit

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/rfq"
```

RFQ live submission is deliberately unsupported today. The package gives SDK
consumers stable request/quote/response DTOs and validates inputs; `Submit`
returns `ErrSubmitUnsupported` after validation (`pkg/rfq/rfq.go:1`,
`pkg/rfq/rfq.go:18`, `pkg/rfq/rfq.go:114`).

```go
var ErrSubmitUnsupported error
...
## `pkg/signers` — Public Signing Seam

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/signers"
```

Stable signer interface plus local secp256k1 adapter for auth, order, and
deposit-wallet typed-data flows (`pkg/signers/signers.go:1`,
`pkg/signers/signers.go:21`). Remote/cloud adapters are optional seams, not CLI
defaults.
...
## `pkg/mcp` — Read-Only Agent Handlers

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/mcp"
```

MCP exposes safe discovery artifacts and read-only handlers only. Tool names
live in `mcp.SafeTools()` (`pkg/mcp/mcp.go:86`). See
...
func NewSDKReadOnlyHandlers(config HandlerConfig, gammaClient *gamma.Client, dataClient *data.Client, clobClient *clob.Client) map[string]ToolHandler
func NewMarketDataSnapshotHandler(tracker *marketdata.Tracker) ToolHandler
```

## `pkg/openapi` — Read-Only OpenAPI Spec

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/openapi"
```

OpenAPI paths live in `openapi.Spec()` and exclude mutating operations
(`pkg/openapi/openapi.go:1`, `pkg/openapi/openapi.go:14`).

```go
func Spec() map[string]any
```

## `pkg/plugins` — Embedder Extension Points

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/plugins"
```

Small interfaces for custom market-data resolution/filtering and pre-trade risk
checks (`pkg/plugins/plugins.go:1`, `pkg/plugins/plugins.go:11`).

```go
type MarketDataPlugin interface { Resolve(...); Filter(...) }
type RiskPlugin interface { CheckOrder(ctx context.Context, order Order) error }
```

## `pkg/capabilities` — Typed Capability Map

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/capabilities"
```

The Capability Map: typed metadata for every Polymarket surface polygolem
exposes — service, auth requirement, wallet mode, and read-only/mutating
classification (`pkg/capabilities/capabilities.go:1`). Single source of truth
for surface claims; `docs/COMPATIBILITY.md` is generated from it.
...
## `pkg/compat` — Compatibility Contract

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/compat"
```

Machine-readable compatibility contract assembled from the Capability Map and
normalized error kinds, for downstream services and UIs
(`pkg/compat/compat.go:1`).
...
## `pkg/polyerrors` — Stable Error Kinds

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/polyerrors"
```

Normalizes upstream Polymarket failures (rate limits, geoblock, auth
rejection, tick-size mismatch, closed markets, insufficient funds, outages)
into stable kinds for operators, agents, and UI adapters
(`pkg/polyerrors/polyerrors.go:1`).
...
## `pkg/reconciliation` — Read-Only Reconciliation Report

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/reconciliation"
```

Builds a Reconciliation Report: read-only status labels and conflict detection
across CLOB order records, Data API positions/trades, relayer transaction
state, and optional on-chain fills (`pkg/reconciliation/reconciliation.go:1`).
It labels observed status and source disagreement; it does not declare
economic truth unless a source authority is named.

```go
func BuildReport(in Input) Report
```

## `pkg/upstreamdrift` — Official Docs Drift Checker

```go
import "github.com/TrebuchetDynamics/polygolem/pkg/upstreamdrift"
```

Checks a saved official Polymarket docs index (`llms.txt`) against the
polygolem compatibility surface (`pkg/upstreamdrift/llms.go:1`). Read-only and
credential-free; backs the `polygolem check-upstream llms` command.
...
| Gap | Current Status |
|---|---|
| Public type ownership | Gamma, Data API, read-only CLOB DTOs, market/user stream DTOs, and authenticated CLOB account/trading DTOs are public; rewards and some enrichment DTOs still need promotion work before claiming a fully clean external SDK |
...
- a public constructor, method, DTO, or error sentinel changes under `pkg/`;
- a package graduates from `pkg/experimental/` into `pkg/`;
- generated command docs or [Architecture](https://github.com/TrebuchetDynamics/polygolem/blob/main/docs/ARCHITECTURE.md) change public SDK boundaries.

## See Also

- [Internal Packages](/docs/reference/internal-packages) — full internal package contracts
- [CLOB V2 API](/docs/reference/clob-api) — upstream REST mapping and signed order contract
- [CLI Commands](/docs/reference/cli) — command-line surface
- [Smart Contracts](/docs/concepts/contracts) — on-chain addresses and ABI
