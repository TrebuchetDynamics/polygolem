...
The single home for the CLI's live-money guards: `EnforceNotionalCap` (the
`POLYGOLEM_MAX_LIVE_ORDER_USD` order cap) and `RequireConfirm` (the typed
confirmation token). Every mutating CLI command routes through it, and a
fail-closed coverage test proves each live-money command rejects a violating
invocation before the private key loads.

```go
import "github.com/TrebuchetDynamics/polygolem/internal/livegate"
// EnforceNotionalCap(notional *big.Rat) error
// RequireConfirm(confirm, token string) error
```
...
- [Go SDK Reference](/docs/reference/sdk) — public `pkg/` packages
- [Architecture](/docs/concepts/architecture) — dependency direction and package boundaries
- [Smart Contracts](/docs/concepts/contracts) — on-chain contract addresses and ABI
