...
The `polygolem markets` family is the CLI interface to Gamma:

```bash
polygolem markets search --query "btc 5m" --limit 10
polygolem markets markets --limit 20 --active
polygolem markets market --id    0xbd31dc8a...
polygolem markets market --slug  will-btc-be-above
polygolem markets enrich --id    0xbd31dc8a...   # joins Gamma + CLOB
polygolem markets tags --limit 100
polygolem markets categories --slug mentions --events --limit 10
polygolem markets series --limit 20
polygolem markets comments --entity-id 123 --entity-type market
```
...
| Events   | List, by ID, by slug, keyset pagination, curated category feeds |
...
| `CategoryEvents` | `GET /events/keyset` | Public | Curated polymarket.com category feeds using `tag_slug` where available; `Breaking` is route-only (`pkg/gamma/categories.go:17` to `pkg/gamma/categories.go:74`) |
...
| Exact website category menu endpoint | No single public endpoint observed; polygolem uses a curated mapping plus Gamma `events/keyset` feeds |
...
<Aside type="note">
**Source of truth** for `polygolem markets` flag semantics is
[`docs/COMMANDS.md`](https://github.com/TrebuchetDynamics/polygolem/blob/main/docs/COMMANDS.md)
(see the `discover` section). Run `polygolem markets --help` for live help.
</Aside>

## Related

- [Market Discovery](/docs/guides/market-discovery) — task-oriented walkthrough
- [Markets, Events & Tokens](/docs/concepts/markets-events-tokens) — data model
