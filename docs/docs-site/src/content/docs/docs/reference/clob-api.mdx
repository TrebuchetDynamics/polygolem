...
```bash
polygolem book get        --token-id <token-id>
polygolem book price      --token-id <token-id>
polygolem book midpoint   --token-id <token-id>
polygolem book spread     --token-id <token-id>
polygolem book tick-size  --token-id <token-id>
polygolem book fee-rate   --token-id <token-id>
polygolem book last-trade --token-id <token-id>

polygolem exchange book <token-id>
polygolem exchange tick-size <token-id>
polygolem exchange price-history <token-id> --interval 1m
polygolem exchange markets --cursor ""
polygolem exchange market <condition-id>
polygolem exchange market-by-token <token-id>
```
...
polygolem exchange create-api-key-for-address --owner <deposit-wallet>
polygolem exchange create-builder-fee-key
polygolem exchange list-builder-fee-keys
polygolem exchange revoke-builder-fee-key --key <builder-code>
...
polygolem exchange order          <order-id>
polygolem exchange trades

polygolem exchange cancel         <order-id>
polygolem exchange cancel-orders  <order-id-1,order-id-2>
polygolem exchange cancel-market  --market <condition-id>
polygolem exchange cancel-market  --asset <token-id>
...
polygolem exchange heartbeat --id keepalive-1
```
...
- [Go SDK Contracts](/docs/reference/sdk) — public package contracts
- [Smart Contracts](/docs/concepts/contracts) — CTF Exchange, pUSD, and wallet factory addresses
- [Deposit Wallets & Signature Types](/docs/concepts/deposit-wallets) — POLY_1271 flow
- [Orderbook Data](/docs/guides/orderbook-data) — public data walkthrough
