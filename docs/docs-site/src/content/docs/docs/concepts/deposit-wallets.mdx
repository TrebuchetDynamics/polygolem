...
Same EOA → same wallet address. Always. Pure local computation via `polygolem wallet derive`.
...
for the full rule and the `wallet status --json` fields
...
Polygolem signs orders with `builder-keys = bytes32(0)` by default. Configure a
...
Use `polygolem wallet settlement-status` as the read-only gate before
...
- [Smart Contracts](/docs/concepts/contracts) — factory addresses and ABI
- [Deposit Wallet Lifecycle](/docs/guides/deposit-wallet-lifecycle) — full end-to-end walkthrough
- [Redeeming Winning Positions](/docs/guides/redeem-winners) — V2 settlement process
- [CLOB API](/docs/reference/clob-api) — order creation and signing
- [Secrets Management](/docs/concepts/secrets-management) — builder credential handling
