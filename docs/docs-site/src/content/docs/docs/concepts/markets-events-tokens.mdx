...
`polygolem markets enrich --id <conditionID>` joins both views into a single
JSON document — see [Market Discovery](/docs/guides/market-discovery).
...
```bash
polygolem book get      --token-id 71321045679...
polygolem book tick-size --token-id 71321045679...
polygolem exchange create-order  --token-id 71321045679... --side BUY ...
```
...
identifier from `markets enrich` first.
</Aside>
...
|----------------------------|-------------|-------|
| `markets search` / `markets market` | Gamma — markets, events | Returns Gamma fields. |
| `markets enrich` | Gamma + CLOB | Joins by `conditionID`. |
| `book *` / `pkg/orderbook` / `pkg/clob` | CLOB — by `tokenID` or `conditionID` | Read-only L0. |
| `exchange create-order` | CLOB — by `tokenID` | Authenticated L2. |
| `pkg/marketresolver.ResolveTokenIDs` | Gamma → both `tokenID`s | High-level resolver. |

## Reference

- [Gamma API](/docs/reference/gamma-api) — full Gamma surface
- [CLOB API](/docs/reference/clob-api) — full CLOB surface
- [Polymarket API Overview](/docs/concepts/polymarket-api) — every API at a glance
