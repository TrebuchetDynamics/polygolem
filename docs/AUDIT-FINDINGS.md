...
| PRD R8 (WebSocket And Streaming) requires authenticated user streams and RTDS isolation | docs/PRD.md | `internal/stream` and `pkg/stream` now expose market and user clients; RTDS isolation remains planned | → docs | Updated in parity pass; `stream user` covers authenticated order/trade inspection |
