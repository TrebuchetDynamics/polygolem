...
| Token IDs / condition IDs | Buying the wrong market/outcome | Resolve current market window and token IDs immediately before user-directed order placement. |
| Market data freshness | Trading stale prices/windows | Bind the caller's decision to a timestamped window; stop on mismatch. |

## Operator checklist before live orders

1. Run read-only discovery and `diag` first.
2. Run the paper flow with the same asset/interval/side.
