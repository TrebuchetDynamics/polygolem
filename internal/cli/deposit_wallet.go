...
func depositWalletCmd(jsonOut bool) *cobra.Command {
	cmd := commandGroup("wallet", "Deposit wallet onboarding (WALLET-CREATE, nonce, batch, status)")
...
// lands on the EOA; chain `polygolem wallet fund --amount X` after
...
it requires --confirm ONBOARD_WALLET to authorize the live-money sequence.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result["nextSteps"] = []string{
				...
				"Verify: polygolem exchange balance --asset-type collateral",
			}
...
func validateEnableTradingReadiness(ctx context.Context, privateKey, wallet, nonce string, clobCredentials clobCredentialReadiness, rpcURL string) (map[string]interface{}, error) {
	...
	if !ready {
		out["nextAction"] = "polygolem wallet onboard"
	}
	return out, nil
}
...
// RELAYER_API_KEY_ADDRESS, generated by `polygolem credentials login`)
...
positions.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(missing) > 0 {
				return printJSON(cmd, map[string]interface{}{
					"ok":               false,
					"error":            "deposit wallet has not approved one or more V2 collateral adapters; run `polygolem wallet approve-adapters --submit --confirm APPROVE_ADAPTERS` first",
