...
// file. Companion to `polygolem builder-keys auto`, which mints CLOB L2 creds.
//
...
func newAuthHeadlessOnboardCommand(jsonOut bool) *cobra.Command {
	...
	cmd.Long = `Compatibility name for 'polygolem credentials login'. It signs the
...
func persistRelayerV2Key(target string, key relayer.V2APIKey, force bool) error {
	...
	body := fmt.Sprintf(
		"# Polymarket V2 relayer credentials\n"+
			"# Generated by polygolem credentials login.\n"+
			"# Created: %s\n"+
			"RELAYER_API_KEY=%s\n"+
