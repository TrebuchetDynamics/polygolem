...
docs/HEADLESS-BUILDER-KEYS-INVESTIGATION.md.`,
	...
	createOrderCmd.Flags().StringVar(&createOrderType, "order-type", "GTC", "order type: GTC (rest on book), GTD (rest until --expiration), FOK (fill full size immediately or cancel), FAK (fill what crosses immediately, cancel the rest)")
	createOrderCmd.Flags().StringVar(&createOrderExpiration, "expiration", "0", "GTD only: unix timestamp when the order expires; must be at least 1 minute in the future")
	...
	marketOrderCmd.Flags().StringVar(&marketOrderAmount, "amount", "", "buy: USDC notional to spend; sell: number of shares to sell")
