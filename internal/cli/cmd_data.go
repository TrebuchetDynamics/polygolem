...
func newDataCommand(reads dataReadRunner) *cobra.Command {
	cmd := commandGroup("analytics", "Polymarket Data API analytics")
...
value, plus market-level holders, open interest, live volume, and leaderboards.`
	cmd.Example = `  polygolem analytics positions --user <wallet-address> --json
  polygolem analytics leaderboard --json`
