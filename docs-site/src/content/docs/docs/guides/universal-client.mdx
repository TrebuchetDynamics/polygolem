...
CLOB credentials are EOA-bound in the validated V2 path; private CLOB account
methods and order placement use the EOA in L2 `POLY_ADDRESS`, while the order
payload carries the deposit wallet as `maker` and `signer`.
